...
bool load_conf_value(const char* key, char* const buffer, size_t buffer_size){
    ...
    if (!getenv(key)){
        ...
        if(scan_buff_size >= buffer_size || scan_buff_size == 0){
            write_logn("buffer too small for %s value, exceeding %d, with size: %d, or can not read file", key, buffer_size, scan_buff_size);
            return false;
        }
        return true;
    }
    ...
    if(env_size >= buffer_size){
        write_logn("buffer too small for %s value, exceeding %d", key, buffer_size);
        return false;
    }
